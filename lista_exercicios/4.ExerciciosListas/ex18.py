...
for c in range(23):
    num = int(input(f"Número do {c} jogador (0=fim): "))
    if num > 23:
        print("Informe um valor entre 1 e 23 ou 0 para sair!")
    if num == 0:
        break

    lista_votos.append(num)

    porcentagem = sum(lista_votos) / len(lista_votos) * 100

print("-" *35)
print("Resultado da votação:\n")
print(f"Foram computados {len(lista_votos)} votos.")
print(f"O melhor jogador foi o número 9, com {max(lista_votos)} votos, correspondendo a {porcentagem:.1f}% do total de votos.")